...
/**
 * Save the current org draft to the backend. Returns the campaign_id.
 * Uses /api/organizations/${orgId}/campaigns/drafts so creator_id = org_id.
 */
export async function saveOrgDraftToBackend(
  orgId: string,
  clerkUser?: ClerkLikeUser | null,
): Promise<number> {
  const { draft } = useOrgCampaignDraft.getState();
  ...
  const doPut = (bearer: string) =>
    fetch(`${API_URL}/api/organizations/${orgId}/campaigns/drafts`, {
      method: "PUT",
      headers: {
  ...
  useOrgCampaignDraft.getState().setCampaignId(campaignId);

  return campaignId;
}

export const useOrgCampaignDraft = create<DraftStore>()(
  persist(
    (set) => ({
      draft: emptyDraft,
