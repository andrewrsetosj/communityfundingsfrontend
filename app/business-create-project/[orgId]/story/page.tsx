...
export default function OrgStoryPage() {
  const { orgId } = useParams<{ orgId: string }>();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { user } = useUser();
  const { draft, setStory } = useOrgCampaignDraft();
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      router.push(`/business-create-project/${orgId}/people?draft=${campaignId}`);
    } catch (err) {
      console.error("Failed to save draft:", err);
      const existingDraft = searchParams.get("draft");
      router.push(
        `/business-create-project/${orgId}/people${existingDraft ? `?draft=${existingDraft}` : ""}`,
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      <div className="space-y-8">
        ...
        <div className="flex justify-between">
          <Link
            href={`/business-create-project/${orgId}/rewards${searchParams.get("draft") ? `?draft=${searchParams.get("draft")}` : ""}`}
